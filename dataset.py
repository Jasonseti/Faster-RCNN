...
class PascalVOC(tf.keras.utils.Sequence):
    ...
    def __init__(self, annot_files, limit=None, shuffle=True, preprocessing=True):
        ...
        self.limit = limit
        self._min_side = params.min_side
        self._max_side = params.max_side
        self._pad_stride = params.stride
        if self.is_shuffle:
    ...
    def __getitem__(self, i):
        ...
        if self.is_preprocessing:
            ...
            image, bboxes = utils.preprocessing.random_resize(image, bboxes, self._min_side, self._max_side)
        image = utils.preprocessing.pad_image(image, _pad_stride=self._pad_stride)
